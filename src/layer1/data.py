...
def gen_prompt_uniform(
    ...
    if seed is not None:
        ...
        local_rd = random.Random(seed)  # For rd.choice calls
        local_np_random = np.random.RandomState(seed)
    else:
        local_random = random
        local_rd = rd
        local_np_random = np.random
    nb_gen = 0
    ioi_prompts = []
    while nb_gen < N:
    ...
    def __getitem__(self, idx):
        if isinstance(idx, list):
            return ([self.data[i] for i in idx],
                    ...
                    [self.subjects[i] for i in idx])
        else:
            return self.data[idx], self.labels[idx], self.subjects[idx]

    def __len__(self):
        return len(self.data)
def get_ioi(nsamples, seed, seqlen, model):
    N=1000
    ...
    corrupted_dataset = (ioi_dataset.gen_flipped_prompts(("S2", "IO")))
    ...
    for i, sentence in enumerate(ioi_dataset.sentences):
        ...
        cleand.append(s)
        cleanlabs.append(l)
    for i, sentence in enumerate(corrupted_dataset.sentences):
        s, l = extract_label(sentence)
        corruptd.append(s)
    ...
    clean_dataset = ObjectData(cleand, cleanlabs, clean_s_words)
    corrupted_dataset = ObjectData(corruptd, corruptlabs, clean_s_words)

    train_size = int(train_ratio * len(clean_dataset))
    validation_size = len(clean_dataset) - train_size

    generator = torch.Generator().manual_seed(seed)
    cltraindata, clvaldata = random_split(clean_dataset, [train_size, validation_size], generator = generator)
    corrtraindata, corrvaldata = random_split(clean_dataset, [train_size, validation_size], generator = generator)
    ...
    corrvalloader = []
    for _ in range(nsamples):
        i = random.randint(0, len(cltraindata) - 1)
        sentence, _, _ = cltraindata[i]
        sentencecor, _, _ = corrtraindata[i]
        
        inp = model.to_tokens(sentence)
        inpcor = model.to_tokens(sentencecor)
        tar = inp.clone()
        tarcor = inpcor.clone()
        
        tar[:, :-1] = 0
        tarcor[:, :-1] = 0
        cleantrainloader.append((inp, tar))
        corrtrainloader.append((inpcor, tarcor))
        i = random.randint(0, len(clvaldata) - 1)
        sentence, _, _ = clvaldata[i]
        sentencecor, _, _ = corrvaldata[i]
        inp = model.to_tokens(sentence)
        inpcor = model.to_tokens(sentencecor)
        cleanvalloader.append(inp)
        corrvalloader.append(inpcor)
    return cleantrainloader, cleanvalloader, corrtrainloader, corrvalloader
# Function to select the appropriate loader based on dataset name
def get_loaders(name, nsamples=128, seed=0, seqlen=20, model=None):
    if 'wikitext2' in name:
        return get_wikitext2(nsamples, seed, seqlen, model)
    if "c4" in name:
        return get_c4(nsamples, seed, seqlen, model)
    if "ioi" in name:
        return get_ioi(nsamples, seed, seqlen, model)
